...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /*heming add for hardware information,begin*/
    else
    {
        #include <linux/hardware_info.h>
		if(0 == Module_ID){		
			sprintf(tp_string_version,"GT9xx_oufeiguang; version:0x%x",grp_cfg_version);
	         }
                 else if(5 == Module_ID){
			sprintf(tp_string_version,"GT9xx_xinli; version:0x%x",grp_cfg_version);
		}
                else
		{
			sprintf(tp_string_version,"GT9xx_undefined; version:0x%x",grp_cfg_version);

		}
		
		hardwareinfo_set_prop(HARDWARE_TP, tp_string_version);
    }
    /*heming add for hardware information,end*/
