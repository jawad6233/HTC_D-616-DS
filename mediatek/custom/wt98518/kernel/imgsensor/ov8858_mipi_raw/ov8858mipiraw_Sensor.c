...
static int read_otp_info(struct otp_struct *otp_ptr)
{
	...
	g_ov8858_module_id = (*otp_ptr).module_integrator_id;
return 0;
}
...
static int read_otp_VCM(int index, struct otp_struct * otp_ptr)
{
	...
	OV8858DB("read_otp_VCM OV8858_VCM_start=%d,OV8858_VCM_end=%d",OV8858_VCM_start,OV8858_VCM_end);
	for (i=start_addr; i<=end_addr; i++) {
	OV8858_write_cmos_sensor(i, 0x00);
	}
return 0;
...
static int update_otp_VCM()
{
	...
	OV8858DB("###update_otp_VCM temp is : %d, i is :%d \n ",temp, i);
	if (temp == 2) {
	otp_index = i;
	break;}
	}
...
OV8858_write_cmos_sensor(0x0302,0x1f);  // ; modify to 744Mbps/lane                                   
...
OV8858_write_cmos_sensor(0x4837,0x15);//0x16 modify mipi timing  720Mbps->744Mbps
...
OV8858_write_cmos_sensor(0x5780,0xfc);//dpc control jason 20140318
OV8858_write_cmos_sensor(0x5781,0xdf);//dpc control 
OV8858_write_cmos_sensor(0x5782,0x3f);//dpc control 
OV8858_write_cmos_sensor(0x5783,0x08);//dpc control 
OV8858_write_cmos_sensor(0x5784,0x0c);//dpc control 
OV8858_write_cmos_sensor(0x5786,0x20);//dpc control 
OV8858_write_cmos_sensor(0x5787,0x40);//dpc control 
OV8858_write_cmos_sensor(0x5788,0x08);//dpc control 
OV8858_write_cmos_sensor(0x5789,0x08);//dpc control  
OV8858_write_cmos_sensor(0x578a,0x02);//dpc control  
OV8858_write_cmos_sensor(0x578b,0x01);//dpc control 
OV8858_write_cmos_sensor(0x578c,0x01);//dpc control 
OV8858_write_cmos_sensor(0x578d,0x0c);//dpc control 
OV8858_write_cmos_sensor(0x578e,0x02);//dpc control 
OV8858_write_cmos_sensor(0x578f,0x01);//dpc control 
OV8858_write_cmos_sensor(0x5790,0x01);//dpc control  
...
OV8858_write_cmos_sensor(0x5b05,0x6c);//dpc control jason 20140318                                                                             
...
UINT32 OV8858GetSensorID(UINT32 *sensorID)
{
	...
	if(read_otp_info(&current_otp)){
		*sensorID = 0xFFFFFFFF;
		OV8858DB("Read Module ID Fail!No valid module info OTP data");
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	OV8858DB("%s Module ID: 0x%x ", __FUNCTION__,g_ov8858_module_id);
	if(g_ov8858_module_id!= 0x11){ 
	*sensorID = 0xFFFFFFFF;
	OV8858DB("Module ID error,g_ov8858_module_id= 0x%x\n", g_ov8858_module_id);
	return ERROR_SENSOR_CONNECT_FAIL;
	}
...
UINT32 OV8858SetVideoMode(UINT16 u2FrameRate)
{
	...
	OV8858DB("[OV8858SetVideoMode]MIN_Frame_length=%d,ov8858.DummyLines=%d,u2FrameRate = %d\n",MIN_Frame_length,ov8858.DummyLines, u2FrameRate);
