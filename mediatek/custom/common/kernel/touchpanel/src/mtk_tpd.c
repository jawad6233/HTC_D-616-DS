...
static struct sb_handler MTK_TS_sb_handler = 
{
    .level = SB_LEVEL_DISABLE_TOUCH,    
    .enable = NULL,
    .disable  = NULL,
};
static void tpd_sb_suspend(struct sb_handler *h)
{
    TPD_DMESG("Smart book plug in, Touch do nothing!\n");
}
static void tpd_sb_resume(struct sb_handler *h)
{
    TPD_DMESG("Smart book plug out!\n");
}
#endif
#endif
...
#ifdef MTK_SMARTBOOK_SUPPORT
#ifdef CONFIG_HAS_SBSUSPEND
    MTK_TS_sb_handler.enable = tpd_sb_suspend;
    MTK_TS_sb_handler.disable = tpd_sb_resume;
    register_sb_handler(&MTK_TS_sb_handler);
#endif	
#endif
    #endif		  
...
#ifdef MTK_SMARTBOOK_SUPPORT
#ifdef CONFIG_HAS_SBSUSPEND
    unregister_sb_handler(&MTK_TS_sb_handler);
#endif	
#endif
    return 0;
}
